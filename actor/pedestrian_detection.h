...
public:
    ...
    Box_t post_process_ssd(const cv::Mat& im, float threshold, const float* outdata, int num)
    {
    ...
    void exec() override {
        ...
        if (result.score > show_threshold) {
            cv::Rect rect(std::max(0, result.x0), std::max(0, result.y0), result.x1-result.x0, result.y1-result.y0);
